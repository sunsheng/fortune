...
function RenderTmpl(config) {
    $.ajax({
        ...
        async: false,
        error: function (err) {
             alert("当前网络异常，请稍后再试(ajax error)：" + err.status);
        },
...
$("#yjfk #btnFeedBack").bind("click", function () {
    ...
    $.ajax({
        ...
        data: feedback,
        error: function (err) {
            alert("脚本执行异常，请刷新页面重试(ajax error)：" + err);
...
function warnMsg(msg,btnText,callback){
	...
	easyDialog.open({
		container : {
			header : '提示信息',
			content :"<p class='tan_main'> <img src='js/common/common/images/icon_t.png'/>"+msg+"</p>",
			yesFn : yesBtn,
			noFn : false,
			yesText:btnText
		},
	...
	//控制样式 easyDialogBox $("div").css({fontSize:"30px",color:"red"})
	$("#easyDialogBox").css({position:"fixed",top:"220px"});
}

/**
 * 封装警告弹出框 关闭按钮带回调函数 正确提示
 * msg：弹出框内容
 */
function warnMsg2(msg,btnText,callback){
	var yesBtn = function(){
		if(callback){
			callback();
		}
	...
	easyDialog.open({
		container : {
			header : '提示信息',
			content :"<p class='tan_main'> <img src='js/common/common/images/icon_o.png'/>"+msg+"</p>",
			yesFn : yesBtn,
			noFn : false,
			yesText:btnText
		},
...
/**
 * 封装警告弹出框 关闭按钮带回调函数 错误提示
 * msg：弹出框内容
 */
function warnMsg21(msg,btnText,callback){
	var yesBtn = function(){
		if(callback){
			callback();
		}
 ...
 * msg：弹出框内容
 */
function confirmChoseMsg(msg,btnText,callback){
	var yesBtn = function(){
		if(callback){
			callback();
		}
	...
	easyDialog.open({
		container : {
			...
			content : //"<p class='tan_main'> <img src='../resources/images/icon_t.png'/>"+msg+"</p>" +
					"<div style='text-align:center;line-height:40px;font-size:14px;margin:0 auto'>" +
					"<font style='margin-left:20px;'>请选择客户经营类型：</font><input type='radio' name='busstype' value='1' checked='checked' style='margin-right: 5px;'>经销" +
					"<input type='radio' name='busstype' value='2' style='margin: 0 5px 0 20px;'>联营</div>" +
					"<div style='text-align:center;line-height:40px;font-size:14px;margin:0 auto'>" +
					"<font style='margin-left:20px;'>请选择合作公司类型：</font><input type='radio' name='branch' value='1' checked='checked' style='margin-right: 5px;'>中商平价" +
					"<input type='radio' name='branch' value='2' style='margin: 0 5px 0 20px;'>中商百货</div>"
					,
			yesFn : yesBtn,
			noFn : true,
			yesText:btnText
...
function errorMsg(msg,btnText,callback){
	...
	easyDialog.open({
		container : {
			header : '提示信息',
			content :"<p class='tan_main'> <img src='js/common/common/images/wrong.png'/>"+msg+"</p>",
			yesFn : yesBtn,
			noFn : false,
			yesText:btnText
		},
...
//列表传入字符串控制展示长度String （传入字符串） length（列表中展示长度）
function formatString(String,length){
	 if(String!=null && String!=""){
		 if(String.length>length){
			 format =  String.substring(0,length)+"...";
		 }
		return String; 
	 }else{
		 return "";
